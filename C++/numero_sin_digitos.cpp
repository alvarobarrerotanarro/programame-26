...
public:
	...
	*/
	std::size_t contarBolas(std::size_t num, std::size_t base) {
		std::size_t total = 0;
		std::size_t digito;
	...
	*/
	std::size_t contarSeparadores(std::size_t num, std::size_t base) {
...
static void casoPrueba() {
	...
	for (int i = 2; i <= 10; i++) {
		actual = aba.contarBolas(num, i) + aba.contarSeparadores(num, i);
		if (actual < record) {
			record = actual;
			min_base = i;
		}
